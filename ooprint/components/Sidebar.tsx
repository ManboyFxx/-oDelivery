...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, tenantProfile }) => {
  const menuItems = [
    {
      id: AppTab.ORDERS,
      label: 'Pedidos',
      ...
      <div className="hidden md:block p-6 border-t border-[#1e293b]">
        <div className="flex items-center gap-3 p-3.5 rounded-xl bg-white/5 border border-white/5 group transition-all">
          ...
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-bold truncate text-gray-200">{tenantProfile?.name || 'ÓoPrint Sistema'}</p>
            <div className="flex items-center gap-2 mt-0.5">
              <div className="w-1.5 h-1.5 rounded-full shadow-[0_0_6px_rgba(255,61,3,0.5)]" style={{ backgroundColor: '#ff3d03' }} />
              <p className="text-[10px] font-bold uppercase tracking-widest truncate" style={{ color: '#ff3d03' }}>
                {tenantProfile?.plan ? `Plano: ${tenantProfile.plan}` : 'v2.4 Online'}
              </p>
            </div>
          </div>
        </div>
