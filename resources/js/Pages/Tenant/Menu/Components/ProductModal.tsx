...
export default function ProductModal({ product, isOpen, onClose, onAddToCart, initialValues }: ProductModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center pointer-events-none">
            ...
            {/* Modal Content */}
            <div className="bg-white w-full md:max-w-2xl max-h-[90vh] md:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden flex flex-col pointer-events-auto animate-in slide-in-from-bottom md:slide-in-from-bottom-10 duration-300">
                {/* Image Header */}
                ...
                {/* Content Scrollable */}
                <div className="flex-1 overflow-y-auto px-6 py-6 relative bg-white">
                    <div className="mb-4">
                            ...
                            <div key={group.id} className="mb-6">
                                <div className="flex items-center justify-between mb-3 sticky top-0 bg-white py-2 z-10 border-b border-gray-100">
                                    ...
                                    <div className={clsx(
                                        ...
                                        isSatisfied ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                                    )}>
                                            ...
                                            <div key={option.id} className={clsx(
                                                ...
                                                {qty > 0 || canAdd ? (
                                                    <div className="flex items-center gap-3 bg-white rounded-lg p-1 shadow-sm border border-gray-100">
                                                        ...
                                                        {qty > 0 && <span className="font-bold text-sm min-w-[1rem] text-center">{qty}</span>}
                                                        <button
                                                            onClick={() => updateComplementQuantity(group.id, option.id, 1, maxQty, group.max_selections)}
                                                            disabled={!canAdd}
                                                            className="p-1 hover:bg-gray-100 rounded text-[#ff3d03] disabled:opacity-30 disabled:cursor-not-allowed"
                                                        >
                                                            <Plus className="h-4 w-4" />
                                                        </button>
                                                    </div>
                                                ) : (
                                                    <div className="h-8 w-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-400">
                                                        <Plus className="h-4 w-4" />
                                                    </div>
                                                )}
                                            </div>
                    ...
                    <div className="mb-6">
                        ...
                        <textarea
                            ...
                            className="w-full p-4 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-[#ff3d03] resize-none text-sm"
                            rows={3}
                        />
                    </div>
                </div>
                ...
                {/* Footer Actions */}
                <div className="p-4 md:p-6 border-t border-gray-100 bg-white md:rounded-b-3xl shrink-0 relative z-20 pb-safe">
                    <div className="flex gap-4">
                        <div className="flex items-center gap-3 bg-gray-100 rounded-xl px-2">
                            <button
                                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                                className="h-10 w-10 flex items-center justify-center text-gray-600 hover:text-black"
                            >
