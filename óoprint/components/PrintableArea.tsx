...
const PrintableArea: React.FC<PrintableAreaProps> = ({ order, config, printer }) => {
  const getSeparator = () => {
    ...
    const width = printer.paperWidth === '80mm' ? 42 : 32;
    return char.repeat(width);
  };

  const containerStyle = {
    width: printer.paperWidth === '80mm' ? '302px' : '219px',
    ...
    <div className="print-only fixed top-0 left-0 z-[9999] bg-white text-black min-h-screen">
      <div style={containerStyle} className={config.upperCaseOnly ? 'uppercase' : ''}>
        ...
        <div className={`text-center mb-2`}>
          <div className="font-bold text-base mb-1">{config.headerText || 'ÓoPrint Sistema'}</div>
          <div>--------------------------------</div>
        </div>
        ...
        <div className="text-center mt-6">
          ...
          <div className="text-[8px] opacity-40 uppercase tracking-widest font-bold">ÓoPrint - Sistema de Impressão v2.4</div>
          <div className="h-12" /> 
