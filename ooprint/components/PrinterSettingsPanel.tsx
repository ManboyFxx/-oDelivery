...
import { generateCalibrationHtml } from '../utils/calibrationHelper';
...
const PrinterSettingsPanel: React.FC<PrinterSettingsPanelProps> = ({ settings, setSettings }) => {
  ...
  const [availablePrinters, setAvailablePrinters] = useState<string[]>(MOCK_PRINTERS);
    ...
    try {
      if (window.electronAPI) {
        ...
        if (printers && printers.length > 0) {
          // Filter for valid printers and map to names
  ...
  const handlePrintCalibration = async () => {
    const html = generateCalibrationHtml(settings.paperWidth);
    if (window.electronAPI) {
      await window.electronAPI.print(html, {
        silent: false, // Calibration should always show dialog for safety
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-[#0d1117] border border-[#1e293b] rounded-[1.5rem] p-8 shadow-xl space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-6 pt-6 border-t border-[#1e293b]">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button
                    onClick={() => handleChange('silentPrinting', !settings.silentPrinting)}
                    className={`flex items-center justify-center gap-2 py-3.5 rounded-xl border transition-all text-[10px] font-bold uppercase tracking-tighter ${settings.silentPrinting ? 'bg-orange-500/10 border-orange-500/30 text-orange-500' : 'bg-slate-800/50 border-slate-700 text-slate-500'}`}
                    title="Ignora o diálogo de impressão do sistema"
                  >
                    🔇 Silenciosa
                  </button>
                </div>
            ...
            {/* Nova Seção: Margens Milimétricas */}
            <div className="pt-6 border-t border-[#1e293b] space-y-6">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Margens de Precisão (mm)</label>
                <button
                  onClick={handlePrintCalibration}
                  className="text-[10px] font-bold text-[#ff3d03] hover:underline"
                >
                  Imprimir Guia de Calibração
                </button>
              </div>
              <div className="grid grid-cols-4 gap-4">
                {[
                  { label: 'Topo', field: 'top' },
                  { label: 'Base', field: 'bottom' },
                  { label: 'Esq.', field: 'left' },
                  { label: 'Dir.', field: 'right' }
                ].map(({ label, field }) => (
                  <div key={field} className="space-y-2">
                    <p className="text-[9px] font-bold text-gray-600 uppercase text-center">{label}</p>
                    <input
                      type="number"
                      min="0" max="20"
                      value={settings.margins?.[field as keyof typeof settings.margins] ?? 0}
                      onChange={(e) => {
                        const val = parseInt(e.target.value) || 0;
                        setSettings(prev => ({
                          ...prev,
                          margins: { ...(prev.margins || { top: 0, bottom: 0, left: 0, right: 0 }), [field]: val }
                        }));
                      }}
                      className="w-full bg-[#161b22] border border-[#1e293b] rounded-lg px-2 py-2 text-white text-center text-sm font-bold focus:border-[#ff3d03] transition-all"
                    />
                  </div>
                ))}
              </div>
